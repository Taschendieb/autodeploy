...
exports.createConfigIfNonExistent = async function() {
    if (!fs.existsSync(__dirname + "/lib/apimap.json")) {
        // Create JSON Schema
        schema = {
            config: {},
            deployments: []
        }
        await fs.writeFile(__dirname + "/lib/apimap.json", JSON.stringify(schema), (err) => {
            if (err) {
                throw err;
            }
        })
...
exports.appendDeployment = async function(deployment) {
    ...
    fs.readFile(__dirname + "/lib/apimap.json", function (err, data) {
        if (err) {
            throw err;
        }
        ...
        fs.writeFile(__dirname + "/lib/apimap.json", JSON.stringify(json), (err) => {
            if (err) {
                throw err;
            }
        })
...
exports.updateDeployments = async function(deployments) {
    exports.createConfigIfNonExistent()
    fs.readFile(__dirname + "/lib/apimap.json", (err, data) => {
        if (err) {
            throw err;
        }
        ...
        fs.writeFile(__dirname + "/lib/apimap.json", JSON.stringify(d), (err) => {
            if (err) {
                throw err;
            }
        })
...
exports.getDeployment = async function(name) {
    return new Promise((resolve, reject) => {
        if (!fs.existsSync(__dirname + "/lib/apimap.json")) {
            ...
            fs.readFile(__dirname + "/lib/apimap.json", async (err, data) => {
                if (err) {
                    throw err;
                } else {
